...
import (
	...
	"github.com/uname-n/surf/errors"
...
const (
	...
	// JavaScriptEnabled instructs a Browser to execute JavaScript on the page.
	JavaScriptEnabled
)
...
type Browsable interface {
	...
	// SetJavaScriptEnabled enables or disables JavaScript execution.
	SetJavaScriptEnabled(enabled bool)

	// JavaScriptEnabled returns whether JavaScript execution is enabled.
...
type Browser struct {
	...
	// jsEngine handles JavaScript execution
	jsEngine *JSEngine
}
...
func (bow *Browser) httpRequest(req *http.Request) error {
	...
	if bow.attributes[JavaScriptEnabled] {
		if bow.jsEngine == nil {
			bow.jsEngine = NewJSEngine(bow)
		}

		// Execute JavaScript in the page
		err = bow.jsEngine.Execute(dom)
		if err != nil {
			// Log JavaScript execution errors but don't fail the request
			fmt.Printf("JavaScript execution error: %v\n", err)
		}
	}
...
// SetJavaScriptEnabled enables or disables JavaScript execution.
func (bow *Browser) SetJavaScriptEnabled(enabled bool) {
	bow.SetAttribute(JavaScriptEnabled, enabled)

	if !enabled {
		// Clear the JS engine when disabling JavaScript
		bow.jsEngine = nil
	}
}

// JavaScriptEnabled returns whether JavaScript execution is enabled.
func (bow *Browser) JavaScriptEnabled() bool {
	return bow.attributes[JavaScriptEnabled]
}
